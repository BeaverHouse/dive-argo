...
이 문서에서는 Multipass로 VM을 생성하고, `k3s` 를 설치해 보겠습니다.

## 사용 가능한 이미지 확인하기

다음 명령어를 입력해 Multipass에서 사용 가능한 이미지를 확인합니다.
...
이제 `k3s` 를 설치하면 됩니다. 여기서는 기본 옵션으로 설치하겠습니다.

```
curl -sfL https://get.k3s.io | sh -
```

이후 `k3s --version`으로 설치를 확인하고, `kubectl`을 사용할 수 있습니다.
...
이를 호스트 컴퓨터에서 제어할 수 있도록 설정해 보겠습니다.
...
`k3s` 설정파일은 `/etc/rancher/k3s/k3s.yaml` 에 있습니다. 이 파일을 호스트로 복사합니다.
...
이제 호스트에서 `kubectl`로 VM 환경을 제어할 수 있습니다.
