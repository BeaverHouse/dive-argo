...
이번에는 간단하게 helm chart를 수정해 보겠습니다.
...
제가 설정한 이름 `huadmin` 은 알맞게 변경해 주세요. `values.yaml`로 따로 분리해서 관리하는 것도 좋습니다.  

그리고 여기서는 Argo helm chart에서 기본 생성하는 ClusterRole을 사용하였습니다.   
Role도 따로 생성하고 싶다면 이에 해당하는 `yaml` 파일을 하나 더 작성해서 RoleBinding에서 연결해 주시면 됩니다.
...
정상적으로 IP가 할당되었고, helm chart에서 바꾼 이름 `myargo`를 기반으로 리소스가 생성된 것을 확인할 수 있습니다.  
