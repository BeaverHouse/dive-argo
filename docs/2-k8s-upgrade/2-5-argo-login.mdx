...
필요한 설정이 끝났으니, Argo Workflow를 수정해 다시 배포하고 로그인까지 수행해 보겠습니다.

## helm chart 수정

이전에는 따로 수정 없이 helm chart를 배포했었습니다.  
이번에는 간단하게 helm chart를 수정해 보도록 합시다.
...
2. Master node에 배포하기 위해 `nodeSelector` 옵션을 변경합니다.  
   ...
   다른 label 설정을 원하시거나, Master node가 아닌 다른 곳에 배포하고 싶다면 설정을 변경하셔도 무방합니다.  
...
SSO 방식을 사용해 타 OAuth 서비스와 연동하여 로그인을 구성할 수도 있지만, 여기서는 token 로그인 방식[^1]을 적용하겠습니다.

token은 기본 계정 외에 새로 계정을 만들어 발행하겠습니다.  
(기본 계정에서 token 추출이 가능할 수도 있지만, 이 문서에서는 설명하지 않습니다)

새로운 계정 설정을 위해 RoleBinding, ServiceAccount, Secret 총 3개의 설정 파일을 작성해야 합니다.  
제가 설정한 `huadmin` 이름은 알맞게 변경해 주세요.  
그리고 여기서는 Argo에서 기본 생성하는 ClusterRole을 이용하였습니다.   
Role도 따로 생성하고 싶다면 이에 해당하는 `yaml` 파일을 하나 더 작성해서 RoleBinding에서 연결해주시면 됩니다.
...
이제 로그인을 위한 토큰을 확인해야 합니다.  
아래의 명령어로 토큰을 확인할 수 있습니다.  
Linux는 그대로 확인하면 되고, Windows의 경우 master VM Shell에 들어가서 확인하시면 됩니다. 그 외 다른 방법을 사용하셔도 됩니다.
...
정상적으로 고정 IP가 할당되었고, helm chart에서 바꾼 이름 `myargo`를 기반으로 리소스가 생성된 것을 확인할 수 있습니다.  
