...
필요한 설정이 끝났으니, Argo Workflow를 수정해 다시 배포하고 로그인까지 진행해 보겠습니다.

## helm chart 수정

이전에는 따로 수정 없이 helm chart를 배포했었습니다.  
...
2. Master node에 배포하기 위해 `nodeSelector` 옵션을 변경합니다.  
   ...
   다른 label 설정을 원하시거나, Master node가 아닌 다른 곳에 배포하고 싶다면 설정을 변경하셔도 됩니다.  
...
3. 접근을 위한 LoadBalancer 설정을 위해 다음과 같이 `values.yaml` 을 수정합니다.
...
새로운 계정 설정을 위해 RoleBinding, ServiceAccount, Secret 총 3개의 설정 파일을 작성해야 합니다.  
제가 설정한 이름 `huadmin` 은 알맞게 변경해 주세요. `values.yaml` 에 따로 분리하는 것도 좋습니다.

그리고 여기서는 Argo helm chart에서 기본 생성하는 ClusterRole을 사용하였습니다.  
Role도 따로 생성하고 싶다면 이에 해당하는 `yaml` 파일을 하나 더 작성하고, RoleBinding에서 연결해 주시면 됩니다.
